    ...
    Markdown.qaFileUpload = function(uploadURL) {
        ...
        this.prompt = function(callback) {
            ...
            that.$template = $('' +
                ...
                '     document.getElementById("upload-file-input").style.display = "block";' +                
                '  }' +                
                '</script>' +                                                
                ...
                '	<h2 style="margin-top:0">Chose an image.</h2>' +
                '</div>' +
                ...
                '	<label class="radio">' +
                '	  <input type="radio" name="input-type" id="input-type-url" value="url" onchange="disableFile();" checked>' +
                '	  From the web' +
                '	</label><br/>' +
                '	<label class="radio">' +
                '	  <input type="radio" name="input-type" id="input-type-file" value="file" onchange="disableUrl();">' +
                '	  From my computer' +
                '	</label><br/><br/>' +
                '   <!-- input type="file" id="upload-file-input" name="upload-file-input" value="Upload" / -->' +
                '	<!-- h3>OR</h3 -->' +
                '   <input type="file" id="upload-file-input" name="upload-file-input" value="Upload" style="display: none; block; width: 60%; margin-right: auto; margin-left: auto;"/>' +                                
